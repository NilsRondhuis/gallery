...
window.addEventListener('keydown', _.throttle(carousel, 200));
...
function carousel(e) {
  for (let i = 0; i < galleryItems.length; i += 1) {
    ...
    if (items.original === refs.lightboxImage.src) {
      offset = i;
    }
  }

  const res = galleryItems.reduce((arr, img) => {
    arr.push(img.original);
    return arr;
  }, []);
  
  if (e.code === "ArrowRight" && offset < 8) {
    refs.lightboxImage.src = res[offset += 1];
  } 
...
//   refs.lightbox.src = res[number + 1];
// }
